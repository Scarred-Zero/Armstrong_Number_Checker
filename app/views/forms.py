...
class ArmstrongForm(FlaskForm):
    min_num = IntegerField('Minimum Number:', validators=[InputRequired(), NumberRange(min=1)],
                           render_kw={'id': 'min_num', 'class': 'form-control', 'value': 'initial',
                                      'placeholder': 'Enter Min Value'})

    max_num = IntegerField('Maximum Number:', validators=[InputRequired(), NumberRange(min=1)],
                           render_kw={'id': 'max_num', 'class': 'form-control', 'value': 'initial',
...
class CheckNumberForm(FlaskForm):
    check_particular_num = IntegerField('Check Number:', validators=[InputRequired(), NumberRange(min=1)],
                                        render_kw={'id': 'check_particular_num', 'class': 'form-control',
...
class ProfileForm(FlaskForm):
    ...
    email = StringField('Email:', validators=[InputRequired(), Length(min=6, max=80), Email()],
                        render_kw={'class': 'form-control'})
